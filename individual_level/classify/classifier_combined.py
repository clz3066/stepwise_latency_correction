...
class SubjectClassify():
    def __init__(self, name_in_mat, training_seed):
    ...
    def testing_dataset(self, subject_name, task):
        '''
        one subject as testing dataset
        '''
        name = self.mat_name_lst[self.name_in_mat] 
        p_name = subject_name + '_{}_PF_cc.mat'.format(task)
        mat = scio.loadmat(os.path.join(self.data_path, p_name))['results']
        X_primed = np.transpose(mat[0, 0][name], (2, 1, 0))     
               
        up_name = subject_name + '_{}_UPF_cc.mat'.format(task)
    ...
    def training(self):
        ...
        '''
        self.save_path = '../../results/individual_classification/{}/combined/{}'.format(self.training_seed, self.name_in_mat)
    ...
    def testing_separate(self, task):
        '''
        ...
        for testing_seed in range(permutations):
            for step_idx in range(5):
                ...
                self.save_path = '../../results/individual_classification/{}/combined/{}'.format(testing_seed, self.name_in_mat)
                
                for self.k in range(10):
                    group = self.groups[self.k]
                    for subject_name in group:
                        X_test, Y_test = self.testing_dataset(subject_name, task=task)
                        dataset_test     = Data.TensorDataset(X_test, Y_test)
        ...
        excel_file_path = 'subject_{}.xlsx'.format(task)
        df.to_excel(excel_file_path, index=False)


    def testing_combined(self):
        '''
        test the models for general purpose
        '''
        subject_test_result = {}
        ...
        for testing_seed in range(permutations):
            for step_idx in range(5):
                ...
                for self.k in range(10):
                    group = self.groups[self.k]
                    for subject_name in group:
                        X_test1, Y_test1 = self.testing_dataset(subject_name, task='Fsp')
                        X_test2, Y_test2 = self.testing_dataset(subject_name, task='Facc')
                        X_test = torch.cat((X_test1, X_test2), 0)
                        Y_test = torch.cat((Y_test1, Y_test2), 0)
        ...
        excel_file_path = 'subject_combined.xlsx'.format()
        df.to_excel(excel_file_path, index=False)
...
if __name__ == "__main__":
    ...
    subject_models = SubjectClassify(name_in_mat=args.name_in_mat, training_seed=args.training_seed)
    subject_models.training()
    # subject_models.testing(task='Facc')
    # subject_models.testing(task='Fsp')
    subject_models.testing_combined()
