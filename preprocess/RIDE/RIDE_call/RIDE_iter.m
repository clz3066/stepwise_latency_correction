function results = RIDE_iter(data,cfg)
    ...
    for initial = 1:1 
        ...
        for j = 1:cfg.comp_num
            ...
            com_c(:,j) = zeros(d1,1);
            com_c1(:,:,j) = zeros(d1,d3);%stimulus-locked matrix
            amp_c(:,j) = zeros(d3,1);
        end
    ...
    trend_i = cfg.comp_num;
    stream_flow = 1:cfg.comp_num;
    for c = 1:cfg.comp_num
        if cfg.comp.name{c}(1) == 'r' 
            trend_i = c-1;
        end
    end
    ...
    for true_iteration = 1:1
        ...
        for iter = 1:cfg.inner_iter
            ...
            for c = stream_flow
                if stop_c(c) == 0
                    ...
                    temp1 = temp0(:,ones(1,d3));
                    temp1(isnan(temp)) = nan;
                    temp1 = (reshape(temp1(~isnan(temp1)),d1,d3));
            ...
            if stop == 1
                for c = stream_flow
                    ...
                    if cfg.final == 1
                        temp(isnan(temp))=0;
                        amp_c(:,c) = mean(com_c(cfg.comp.twd{c}(1):cfg.comp.twd{c}(2),c*ones(1,d3)).*temp(max_latency(c)+[cfg.comp.twd{c}(1):cfg.comp.twd{c}(2)],:));
                    end
        ...
        if exist('ms','var') 
            results.comp_ms = com_ms;
            results.comp_ms1 = mean(com_ms1,2);
        end
        results.amp = amp_c;
        results.comp = com_c;
