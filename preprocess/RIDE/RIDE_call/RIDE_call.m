...
if cfg.prg == 1
    figure;set(gcf,'menubar','none');axis([0,100,0.5,1.5]);axis off;
    final_correlations = get(gcf,'position');
    set(gcf,'position',[final_correlations(1),final_correlations(2),final_correlations(3),150]);pause(0.001);
end
...
for section = 1:1 
    ...
    for iter = 1:outer_iter
         ...
         for sec_RIDE_inner_iter = 1:1
             ...
             c_l = zeros(d1, d2, cfg.comp_num);%c_l is the latency synced RIDE component
             c_sl = c_l;%c_sl is the stimulus synced RIDE component
             for channel = 1:d2
                 
                 rst = RIDE_iter(squeeze(data(:,channel,:)), cfg1); %RIDE decomposition
                 ...
                 if isfield(cfg,'latency_a')%only for microsaccade 
                     comp_ms(:,channel) = rst.comp_ms;comp_ms1(:,channel) = rst.comp_ms1;
                 end               
             end
             comp = c_l;
             comp1 = c_sl;
         ...
         for section1 = 1:1
            for c_idx = 1:n_of_c
                ...
                for l = 1:d3 %trial_index
                    if l_change(l,c_idx)==1
                        single_trial = data(:,:,l);
                        for j = 1:cfg.comp_num
                            if j~=c_i(c_idx)
                                single_trial = single_trial - move2(comp(:,:,j),cfg.comp.latency{j}(l),'1d');
                            end
                        end
                        if isfield(cfg,'latency_a')%remove the ms component
                            for a = 1:length(find(~isnan(cfg.latency_a(:,l))))
                                    single_trial = single_trial - move2(comp_ms,cfg.latency_a(a,l),'1d'); 
                            end
                        end
                        ...
                        if isfield(cfg,'template') %only for specified tempalte matching
                            if isfield(cfg.template,'chan')
                                channels = cfg.template.chan;
                            end
                        end

                        for channel = 1:length(channels)%cross correlation
                            correlations(:,channel) = xcov(single_trial(:,channels(channel)),current_template(:,channels(channel)),fix(size(single_trial,1)/2),cfg.xc);
                        end

                        mean_correlations{c_idx}(:,l) = mean(correlations,2);

                        %low pass filter the cross correlation curve
                        mean_correlations{c_idx}(:,l) = filtering10(mean_correlations{c_idx}(:,l),1,round(10*cfg.high_cutoff*size(single_trial,1)*cfg.re_samp/1000));
                    end
                end

                %detrend the cross-correlation curve to make sure the latency will not be found on the boundaries
                mean_correlations{c_idx} = RIDE_detrend(mean_correlations{c_idx}(:,:),[1,fix(size(mean_correlations{c_idx},1)*cfg.bd),fix(size(mean_correlations{c_idx},1)*(1-cfg.bd)),size(mean_correlations{c_idx},1)]);
                final_correlations = [];
                correlation_interval = mean_correlations{c_idx}(fix(size(mean_correlations{c_idx},1)/2-cfg.dur{c_i(c_idx)})+1:fix(size(mean_correlations{c_idx},1)/2+cfg.dur{c_i(c_idx)}),:);
                dur_min = fix(size(mean_correlations{c_idx},1)/2-cfg.dur{c_i(c_idx)})+1;
                dur_max = fix(size(mean_correlations{c_idx},1)/2+cfg.dur{c_i(c_idx)});
                fprintf('dur_min: %d, dur_max: %d', dur_min, dur_max);
                ...
                for j = 1:d3 %track the correlation values
                ...
                if iter>1
                 ...
                 index = (l_change(:,c_idx)==1&c_change(:,c_idx)==1);
                 cfg.comp.latency{c_i(c_idx)}(index) = final_correlations(index);
                 cfg.comp.latency{c_i(c_idx)} = round(cfg.comp.latency{c_i(c_idx)}-median(cfg.comp.latency{c_i(c_idx)}));
